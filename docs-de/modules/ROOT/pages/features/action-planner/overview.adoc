...
====
*Plan kopieren:* Anstatt einen leeren neuen Plan zu erstellen, kann über die "Plan kopieren" - Funktion ein Plan erstellt werden, bei dem alle Informationen (Template, Herkunftsdörfer, Zieldörfer, Abschickzeiten, Ankunftszeiten etc.) des ursprünglichen Plans übernommen werden.
...
Die Planübersicht zeigt eine *Bereitschaftscheckliste*, die auf einen Blick anzeigt, welche Konfigurationsschritte bereits abgeschlossen sind:

* Herkunftsdörfer definiert
* Zieldörfer definiert
* Abschickzeiten eingetragen
* Ankunftszeiten eingetragen

Sobald alle Punkte erfüllt sind, kann die Berechnung des Plans gestartet werden.
...
Das kann z.B. für Katapulte das geplante Ziel sein, oder der abgeknickte Speer für Fake-Unterstützungen.
...
IMPORTANT: Damit diese Berechnungsmöglichkeit funktioniert, muss unter xref:features/action-planner/send-and-arrival-time.adoc#abhaengigkeiten_abschickzeiten[Abschickzeiten->Abhängigkeiten] eine Abhängigkeit zu einem anderen Plan eingetragen sein.
...
Unter "Veröffentlichungen" werden alle bisherigen Kampagnen aufgelistet.
Für jede Kampagne wird angezeigt, wie weit der Versandfortschritt ist — also wie viele Accounts bereits mit einem Abschickplan versorgt wurden.
Der Fortschritt wird in Echtzeit aktualisiert.
...
=== Anpassungsaufgaben (Adjustment Tasks)[[adjustment_tasks]]

[TIP,role=premium_silver]
====
Dieses Feature steht nur TW-Forge Silver und TW-Forge Gold Accounts zur Verfügung.
====

Über *Anpassungsaufgaben* können Ankunftszeiten für einzelne Accounts nach der Veröffentlichung gezielt nachjustiert werden.
Für jeden Account kann eine separate Aufgabe erstellt werden, in der eine neue Zielankunftszeit definiert wird.
Die Anpassungsaufgabe wird nach Erstellung angezeigt und kann bearbeitet oder gelöscht werden.

Typischer Anwendungsfall: Ein Account hat geadelt oder war offline — über eine Anpassungsaufgabe kann sein Abschickfenster nachträglich verschoben werden, ohne die gesamte Kampagne neu berechnen zu müssen.
