...
Wenn man mehrere verschiedene Off-, AG, Kata-, UT- oder Fakepläne erstellen möchte, können diese als Gruppen zusammengefasst werden, um die Übersichtlichkeit in der Planung selbst zu gewährleisten.
...
Die planende Person kann statt fixen Truppen auch dynamische Parameter verwenden.
...
Diese Funktion ist für die Berechnung von Zwischencleanern (knapp vor AGs) sowie Unterstützungen/Fake-Unterstützungen (knapp hinter den AGs) gedacht. Der Unterschied zur Standard-Berechnung besteht darin, dass man hier nicht nur ein (fixes) Template angeben kann, sondern mehrere Templates, die sich in Truppenmenge und der Auswahl an Truppen unterscheiden können.
...
Nachdem man das getan hat, muss die planende Person die Offset-Einstellung vornehmen.
...
Nachdem all diese Einstellungen erledigt sind, kann die planende Person Cleaner berechnen.
...
Wenn die planende Person alle zur Aktion benötigten Pläne erstellt hat, so muss er diese veröffentlichen, damit sie für die Member verfügbar werden.
Dafür öffnet sie über das linksseitige aufklappbare Menü und klicke auf "Veröffentlichungen".
Hier kann eine neue Veröffentlichung erstellt werden.

Hierzu gibt sie einen Aktionsnamen, eine Beschreibung sowie ein Nachrichtentemplate ein und bestimmt, für welches Stammeskonstrukt die Aktion geplant wurde.

=== Nachrichtentemplate

Im Nachrichtentemplate verfasst die planende Person eine Nachricht, die als Vorlage für das Verschicken der Aktions-PN als Ingame-Nachricht dient.
...
Nachdem die planende Person die Aktion erstellt hat, hat er zunächst die Möglichkeit, die Abschickpläne jedes einzelnen Accounts in TW-Forge zu überprüfen.
...
So hat die planende Person folgende Möglichkeiten:
